...
export const getAllPromo = (): Promise<QueryResult<IDataSizes>> => {
  const query = `select * from sizes`;
  return db.query(query);
};

export const getOnePromo = (uuid: string): Promise<QueryResult<IDataSizes>> => {
  const query = `select * from sizes where uuid=$1`;
  const values = [uuid];
  return db.query(query, values);
};

export const createPromo = (body: ISizesBody): Promise<QueryResult<IDataSizes>> => {
...
export const deletePromo = (uuid: string): Promise<QueryResult<IDataSizes>> => {
...
export const updateOnePromo = (body: ISizesBody, uuid: string): Promise<QueryResult<IDataSizes>> => {
