...
import { getOnePromo, createPromo, getAllPromo, deletePromo, updatePromo } from "../repositories/promo";
...
export const updatedPromo = async (req: Request<IpromoParams, {}, IpromoBody>, res: Response) => {
  try {
    const result = await updatePromo(req.params, req.body);
    if (result.rowCount === 0) {
      return res.status(404).json({
        msg: "Produk tidak ditemukan",
        data: [],
